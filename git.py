...
Original file is located at
    https://colab.research.google.com/drive/1VBYBWINIhM1exZKw-swh1aeX8Ib60bMZ
"""
...
def gdelt_timeline_matrix(query, timespan="30d"):
    try:
        ...
        if r.status_code == 429:
            print("GDELT rate limit hit (HTTP 429). Continuing with empty GDELT timeline fallback.")
            return pd.DataFrame()
        r.raise_for_status()
        js = r.json()
        tl = js.get("timeline", [])
        ...
        print(f"GDELT timeline unavailable ({type(e).__name__}). Continuing with empty GDELT timeline fallback.")
        return pd.DataFrame()
...
# files.download(zip_path)  # disabled: avoids auto-downloading bu.zip during notebook run
...
def gdelt_doc_for_iso2(iso2: str, max_records=100) -> pd.DataFrame:
    url = "https://api.gdeltproject.org/api/v2/doc/doc"
...
def fetch_all_themes(iso3_to_iso2: dict, max_workers=2) -> pd.DataFrame:
    out=[]
